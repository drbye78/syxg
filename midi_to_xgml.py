...
class MIDItoXGMLConverter:
    ...
    def convert_midi_to_xgml(self, midi_file_path: str) -> dict[str, Any] | None:
        """
    ...
    def _extract_basic_messages(self, messages: list[MIDIMessage]) -> dict[str, Any] | None:
    ...
    def _extract_sequences(self, messages: list[MIDIMessage]) -> dict[str, Any] | None:
        ...
        for channel, events in channel_sequences.items():
            ...
            sequences[sequence_name] = {
                ...
                "time_signature": "4/4",
                "quantization": "1/8",
                "tracks": [{"track": {"channel": channel, "events": events}}],
            }
            track_num += 1

        return sequences

    def _midi_message_to_xgml_event(self, msg: MIDIMessage) -> dict[str, Any] | None:
        """Convert MIDI message to XGML event."""
        if msg.type == "note_on":
            return {
                "at": {
            ...
            if controller_name:
                value = self._midi_controller_value_to_semantic(controller_name, msg.value)
                return {"at": {"time": msg.time, controller_name: value}}

        elif msg.type == "program_change":
            program_name = self._midi_program_to_name(msg.program)
            return {"at": {"time": msg.time, "program_change": program_name}}
    ...
    def _midi_controller_to_name(self, controller: int) -> str | None:
    ...
    def _midi_controller_value_to_semantic(
        self, controller: str, value: int
    ) -> str | int | bool:
...
def get_output_path(
    input_file: str, output: str | None, output_dir: str | None, multiple_files: bool
) -> str:
...
def main():
    ...
    multiple_files = (
        len(args.input_files) > 1 or "*" in str(args.input_files) or "?" in str(args.input_files)
    )
