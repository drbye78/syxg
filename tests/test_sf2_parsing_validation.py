...
class TestSF2PresetStructure:
    ...
    def test_preset_names_match(self, sf2_data):
        ...
        for i, (our_header, baseline_preset) in enumerate(
            zip(our_headers, baseline_presets[:10])
        ):  # First 10
...
class TestSF2InstrumentStructure:
    ...
    def test_instrument_names_match(self, sf2_data):
        ...
        for i, (our_header, baseline_inst) in enumerate(
            zip(our_headers[:10], baseline_instruments[:10])
        ):
            our_name = our_header["name"].strip()
...
class TestSF2SampleMetadata:
    ...
    def test_sample_names_match(self, sf2_data):
        ...
        for i, (our_header, baseline_sample) in enumerate(
            zip(our_headers[:20], baseline_samples[:20])
        ):
            our_name = our_header["name"].strip()
        ...
        for (bank, prog), preset in multizone:
            ...
            for i, zone1 in enumerate(zones):
                for zone2 in zones[i + 1 :]:
                    ...
                    key_overlap = (
                        zone1.key_range[0] <= zone2.key_range[1]
                        and zone2.key_range[0] <= zone1.key_range[1]
                    )
