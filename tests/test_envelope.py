...
class TestBasicEnvelopeGeneration:
    ...
    def test_hold_phase(self, default_envelope):
        ...
        total_skip = default_envelope.delay_samples + int(default_envelope.attack * default_envelope.sample_rate)
        samples_processed = 0

        while samples_processed < total_skip:
            output = np.zeros(min(default_envelope.block_size, total_skip - samples_processed), dtype=np.float32)
            default_envelope.generate_block(output)
            samples_processed += len(output)
    ...
    def test_idle_state_after_release(self):
        ...
        env = UltraFastADSREnvelope(
            ...
            decay=0.01, sustain=0.5, release=0.01,
...
class TestBlockProcessing:
    ...
    def test_block_spans_multiple_phases(self):
        ...
        env = UltraFastADSREnvelope(
            delay=0.001, attack=0.001, hold=0.001,
            decay=0.001, sustain=0.5, release=0.001,
            sample_rate=48000, block_size=1024
        )
    ...
    def test_very_large_block(self):
        ...
        env = UltraFastADSREnvelope(
            delay=0.01, attack=0.01, hold=0.01,
            decay=0.1, sustain=0.5, release=0.1,
            sample_rate=48000, block_size=8192
        )
...
class TestParameterModulation:
    ...
    def test_multiple_simultaneous_modulations(self, default_envelope):
        ...
        assert default_envelope.target_sustain == np.clip(default_envelope.base_sustain + 0.1, 0.0, 1.0)
...
class TestMultiRateSupport:
    ...
    def test_sample_rate_44100(self):
        ...
        env = UltraFastADSREnvelope(
            attack=0.01, sample_rate=44100, block_size=1024
        )
        env.note_on(velocity=100)
    ...
    def test_sample_rate_96000(self):
        ...
        env = UltraFastADSREnvelope(
            attack=0.01, sample_rate=96000, block_size=2048
        )
        env.note_on(velocity=100)
...
class TestEdgeCases:
    ...
    def test_very_long_release(self):
        ...
        env = UltraFastADSREnvelope(
            delay=0.0, attack=0.001, hold=0.0,
            decay=0.001, sustain=0.5, release=10.0,
            sample_rate=48000, block_size=1024
        )
        env.note_on(velocity=100)
...
class TestPerformanceBenchmarks:
    ...
    def test_multiple_envelopes_throughput(self):
        ...
        num_envelopes = 100
        envelopes = [
            UltraFastADSREnvelope(sample_rate=48000, block_size=1024)
...
class TestSF2Integration:
    ...
    def test_envelope_imported_by_sf2_region(self):
        ...
        from synth.core.envelope import UltraFastADSREnvelope, EnvelopeState
