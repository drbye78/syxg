...
if TYPE_CHECKING:
    ...
    from .midi_learn import MIDILearn, LearnTargetType
...
class StyleModulationIntegration:
    ...
    def _on_modulation_cc(self, param: str, normalized_value: float, 
    ...
    def add_modulation_mapping(self, param_name: str, cc_number: int, 
                               min_val: float, max_val: float, 
                               curve: str = 'linear'):
        """Add custom modulation mapping."""
        from .midi_learn import MIDILearnMapping, LearnTargetType
...
class StyleSequencerIntegration:
    ...
    def sync_with_style_section(self):
        ...
        self.style_player.set_section_change_callback(
            self._on_style_section_change
        )
...
class StyleIntegrations:
    ...
    def _initialize_integrations(self):
        ...
        if self.effects and self.dynamics:
            self.integrations['effects'] = StyleEffectsIntegration(
                self.effects, self.dynamics
            )
        
        # 2. Voice ↔ OTS
        if self.voice_manager and self.ots:
            self.integrations['voice'] = StyleVoiceIntegration(
                self.voice_manager, self.ots
            )
        ...
        if self.mpe and self.scale_detector:
            self.integrations['mpe'] = StyleMPEIntegration(
                self.mpe, self.scale_detector
            )
