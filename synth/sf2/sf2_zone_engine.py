...
class SF2ZoneEngine:
    ...
    def get_modulated_parameters(
        self, note: int, velocity: int, controllers: dict[int, float] | None = None
    ) -> dict[str, float]:
        """
    ...
    def _get_source_value(
        self, src_oper: int, note: int, velocity: int, controllers: dict[int, float] | None
    ) -> float:
        """
    ...
    def _apply_to_destination(
        self, params: dict[str, float], dest_oper: int, amount: float
    ) -> None:
        """
