...
class TimeStretchingEngine:
    ...
    def _process_phase_vocoder(self, input_audio: np.ndarray) -> np.ndarray:
        """
        Process using phase vocoder algorithm.
        ...
        # For now, implement simple resampling-based time stretching
        ...
        stretched = np.interp(x_new, x_old, input_audio)

        # Apply pitch shifting if needed
        if self.pitch_ratio != 1.0:
            stretched = self._apply_pitch_shift(stretched, self.pitch_ratio)
