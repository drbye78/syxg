...
class SFF2Section:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "tempo": self.tempo,
            "tracks": {
                TRACK_TYPE_MAP.get(k, f"track_{k}"): v.to_dict() for k, v in self.tracks.items()
            },
        }
...
class SFF2ChordTable:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            chord_key: {
                TRACK_TYPE_MAP.get(k, f"track_{k}"): v for k, v in self.track_voicings.items()
            }
        }
...
class SFF2Parser:
    ...
    def _to_style(self) -> Any:
        ...
        from ..style.style_ots import OneTouchSettings, OTSPart, OTSPreset
...
class SFFGEParser(SFF2Parser):
    ...
    def get_guitar_chord_diagram(self, chord_root: int, chord_type: int) -> dict | None:
        ...
        for table in self.chord_tables:
            if (
                table.chord_root == chord_root
                and table.chord_type == chord_type
                and hasattr(table, "guitar_data")
            ):
