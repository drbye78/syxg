...
class XGEffectsCoordinator:
    ...
    def process_channels_to_stereo_zero_alloc(self, input_channels: List[np.ndarray],
        ...
        with self.lock:
            ...
            try:
                ...
                processed_channels = self._apply_insertion_effects_to_channels(
                    input_channels, num_samples
                )

                # STEP 2: MIX CHANNELS WITH PANNING AND CREATE EFFECT SENDS
                mix_result = self._mix_channels_with_effect_sends(
                    processed_channels, num_samples
                )

                # STEP 3: APPLY VARIATION EFFECTS TO MIX
                variation_output = self._apply_variation_effects_to_mix(
                    mix_result['main_mix'], num_samples
                )

                # STEP 4: APPLY SYSTEM EFFECTS (REVERB/CHORUS) WITH SENDS
                system_output = self._apply_system_effects_with_sends(
                    variation_output, mix_result['reverb_send'],
                    mix_result['chorus_send'], num_samples
                )

                # STEP 5: MASTER FINALIZATION WITH WET/DRY MIXING
                self._apply_master_processing(system_output, num_samples, output_stereo)
    ...
    def _apply_insertion_effects_to_channels(self, input_channels: List[np.ndarray],
        ...
        for ch_idx, channel_data in enumerate(input_channels):
            if ch_idx >= len(self.insertion_effects):
                # No insertion effects for this channel
                processed_channels.append(channel_data)
                continue

            # Get a working buffer for processing
            with self.buffer_manager as bm:
                # Allocate working buffer (will be automatically returned)
                if channel_data.ndim == 1:
                    # Mono input - allocate stereo buffer for processing
                ...
                # Apply insertion effects chain
                insertion_params = {"enabled": True}  # Could be expanded
                ...
                # Store result
                processed_channels.append(working_buffer.copy())
    ...
    def _apply_system_reverb(self, reverb_send: np.ndarray, num_samples: int) -> Optional[np.ndarray]:
        ...
        try:
            # Create a temporary buffer for reverb processing
            temp_buffer = np.zeros((num_samples, 2), dtype=np.float32)
            temp_buffer[:num_samples] = reverb_send
    ...
    def _apply_system_chorus(self, chorus_send: np.ndarray, num_samples: int) -> Optional[np.ndarray]:
        ...
        try:
            # Create a temporary buffer for chorus processing
            temp_buffer = np.zeros((num_samples, 2), dtype=np.float32)
            temp_buffer[:num_samples] = chorus_send
