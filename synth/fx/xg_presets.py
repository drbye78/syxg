...
class XGMasterCompressor:
    ...
    def _update_envelope_coefficients(self) -> None:
        ...
        self.peak_envelope['release_coeff'] = 1.0 - math.exp(-1.0 / (release_time * self.sample_rate))
    ...
    def process_sample(self, input_sample: float) -> float:
        ...
        with self.lock:
            ...
            output_sample = input_sample * (1.0 - self.params['dry_wet']) + wet_sample * self.params['dry_wet']
        ...
        with self.lock:
            for i in range(num_samples):
                ...
                if self.lookahead_delay_samples > 0:
                    ...
                    self.lookahead_write_pos = (self.lookahead_write_pos + 1) % len(self.lookahead_buffer)

                    # Read from delayed position
                    delayed_pos = (self.lookahead_write_pos - self.lookahead_delay_samples) % len(self.lookahead_buffer)
...
class XGMasterSection:
    ...
    def process_block(self, stereo_block: np.ndarray, num_samples: int,
                     input_level: float = 1.0) -> None:
        """
    ...
    def apply_preset_to_coordinator(cls, preset_id: int, coordinator) -> bool:
        ...
        try:
            ...
            if reverb:
                ...
                coordinator.set_system_effect_parameter('reverb', 'pre_delay', reverb.get('pre_delay', 0.02))
                coordinator.set_system_effect_parameter('reverb', 'hf_damping', reverb.get('hf_damping', 0.5))
                coordinator.set_system_effect_parameter('reverb', 'density', reverb.get('density', 0.8))
                coordinator.set_system_effect_parameter('reverb', 'early_level', reverb.get('early_level', 0.5))
                coordinator.set_system_effect_parameter('reverb', 'tail_level', reverb.get('tail_level', 0.5))
            ...
            if chorus:
                ...
                coordinator.set_system_effect_parameter('chorus', 'feedback', chorus.get('feedback', 0.3))
