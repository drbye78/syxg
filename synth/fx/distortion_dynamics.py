...
class DistortionDynamicsProcessor:
    ...
    def _process_step_phaser(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('step_phaser', {
            'lfo_phase': 0.0,
            'step_position': 0,
            'step_size': 0.1,
            'allpass_filters': [0.0] * 12
        })
        ...
        for i in range(num_samples):
            ...
            state['lfo_phase'] = (state['lfo_phase'] + 2 * math.pi * step_freq / self.sample_rate) % (2 * math.pi)
    ...
    def _process_step_flanger(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('step_flanger', {
    ...
    def _process_step_tremolo(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('step_tremolo', {
            'lfo_phase': 0.0,
            'step_position': 0
        })
    ...
    def _process_step_pan(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('step_pan', {
            'lfo_phase': 0.0,
            'step_position': 0
        })
    ...
    def _process_step_filter(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('step_filter', {
            'lfo_phase': 0.0,
            'step_position': 0,
        ...
        for i in range(num_samples):
            state['lfo_phase'] = (state['lfo_phase'] + 2 * math.pi * base_freq / self.sample_rate) % (2 * math.pi)
    ...
    def _process_auto_filter(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('auto_filter', {
            'envelope': 0.0,
            'cutoff': 1000.0,
            'prev_input': 0.0,
    ...
    def _process_compressor_electronic(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('compressor_electronic', {
            'envelope': 0.0,
            'gain': 1.0
        })
        ...
        for i in range(num_samples):
            ...
            if target_gain < state['gain']:
                state['gain'] = state['gain'] - (state['gain'] - target_gain) * attack * self.sample_rate
            else:
                state['gain'] = state['gain'] - (state['gain'] - target_gain) * release * self.sample_rate
    ...
    def _process_compressor_optical(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('compressor_optical', {
            'envelope': 0.0,
            'gain': 1.0
        })
    ...
    def _process_limiter(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('limiter', {
            'envelope': 0.0,
            'gain': 1.0
        })
    ...
    def _process_multi_band_compressor(self, stereo_mix: np.ndarray, num_samples: int,
        ...
        state = self._ensure_state('multi_band_comp', {
            'low_env': 0.0,
            'high_env': 0.0,
            'low_gain': 1.0,
            'high_gain': 1.0,
        ...
        for i in range(num_samples):
            for ch in range(2):
                ...
                high_signal = high_alpha * (sample - low_signal) + (1 - high_alpha) * state['high_filter']
