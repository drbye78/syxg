...
class S90S70PresetCompatibility:
    ...
    def __init__(self):
        ...
        # Hardware profile for parameter clamping
        self.current_hardware_profile = "S90"  # Default to S90
    ...
    def save_preset(
        self, preset_number: int, preset_data: dict[str, Any], bank_name: str | None = None
    ) -> bool:
        """
        Save preset to bank.
    ...
    def load_preset(
        self, preset_number: int, bank_name: str | None = None
    ) -> dict[str, Any] | None:
        """
    ...
    def copy_preset(
        self, source_bank: str, source_preset: int, dest_bank: str, dest_preset: int
    ) -> bool:
        """
    ...
    def export_bank_to_file(self, bank_name: str, filename: str) -> bool:
        ...
        export_data = {"bank_info": bank.get_bank_info(), "presets": bank.presets.copy()}
    ...
    def _validate_parameter_value(self, value: Any, spec: Any) -> bool:
        ...
        if isinstance(spec, tuple) and len(spec) == 2:
            ...
            if not isinstance(value, list) or len(value) != len(spec):
                return False
            return all(
                self._validate_parameter_value(v, s) for v, s in zip(value, spec, strict=False)
            )
    ...
    def _apply_parameter_clamping(self, param_name: str, param_value: Any) -> Any:
        ...
        if param_name in clamping_rules:
            hardware_ranges = clamping_rules[param_name]
            if self.current_hardware_profile in hardware_ranges:
                min_val, max_val = hardware_ranges[self.current_hardware_profile]
                if isinstance(param_value, (int, float)):
                    return max(min_val, min(max_val, param_value))

        # Additional common clamping for all hardware
        if isinstance(param_value, (int, float)):
            # Ensure value is within MIDI range
            return max(0, min(127, int(param_value)))

        return param_value

    def set_hardware_profile(self, profile: str) -> bool:
        """
        Set the current hardware profile for parameter clamping.

        Args:
            profile: Hardware profile name ('S70', 'S90', 'S90ES')

        Returns:
            True if profile was set successfully
        """
        valid_profiles = ["S70", "S90", "S90ES"]
