...
class S90S70HardwareSpecs:
    ...
    def apply_hardware_filter_characteristics(
        self, filter_type: str, frequency: float, resonance: float
    ) -> tuple[float, float]:
        """
        ...
        adjusted_freq = max(
            specs["frequency_range"][0], min(specs["frequency_range"][1], frequency)
        )

        # Apply hardware-specific resonance scaling
        resonance_range = specs["resonance_range"]
        adjusted_resonance = (
            resonance * (resonance_range[1] - resonance_range[0]) + resonance_range[0]
        )
    ...
    def get_hardware_compatible_parameter_range(
    ...
    def simulate_hardware_voice_allocation(
        self, requested_voices: int, voice_type: str = "awm"
    ) -> int:
        """
        ...
        if voice_type == "awm":
            ...
            max_voices = (
                self.current_profile.an_polyphony if self.current_profile.an_engines > 0 else 0
            )
