...
class JupiterXAnalogPlugin(SynthesisFeaturePlugin):
    ...
    def configure_multi_stage_envelope(
        ...
        logger.info(
            "Multi-stage envelope: %d stages, loop %s (%s mode)",
            self.envelope_stages,
            "enabled" if self.envelope_loop_enabled else "disabled",
            self.envelope_sustain_mode,
        )
    ...
    def set_envelope_curves(
        ...
        logger.info(
            "Envelope curves: A=%s, D=%s, S=%s, R=%s",
            self.attack_curve,
            self.decay_curve,
            self.sustain_curve,
            self.release_curve,
    ...
    def set_envelope_levels_times(
        ...
        logger.debug("Envelope levels: %s", self.envelope_levels[: self.envelope_stages])
        logger.debug("Envelope times: %s", self.envelope_times[: self.envelope_stages])
    ...
    def set_velocity_sensitivity_per_stage(
        ...
        logger.info(
            "Velocity sensitivity per stage: A=%.2f, D=%.2f, S=%.2f, R=%.2f",
            self.velocity_to_attack,
            self.velocity_to_decay,
            self.velocity_to_sustain,
            self.velocity_to_release,
        )
    ...
    def set_envelope_retrigger_mode(
        ...
        logger.info(
            "Envelope retrigger: %s mode, legato %s",
            self.envelope_retrigger_mode,
            "enabled" if self.envelope_legato_mode else "disabled",
        )
    ...
    def set_envelope_key_tracking(
        ...
        logger.info(
            "Envelope key tracking: %.2f (center: %d), velocity curve: %s",
            self.envelope_key_tracking,
            self.envelope_key_center,
            self.envelope_velocity_curve,
        )
    ...
    def load(self, context: PluginLoadContext) -> bool:
        ...
        try:
            ...
            logger.info("Jupiter-X Analog Extensions loaded")
            return True

        except Exception as e:
            logger.error("Failed to load Jupiter-X analog extensions: %s", e)
            return False
    ...
    def unload(self) -> bool:
        ...
        try:
            ...
            logger.info("Jupiter-X Analog Extensions unloaded")
            return True

        except Exception as e:
            logger.error("Error unloading Jupiter-X analog extensions: %s", e)
            return False
    ...
    def enable_hard_sync(self, enabled: bool = True):
        ...
        logger.info("Hard sync %s", "enabled" if enabled else "disabled")
    ...
    def enable_soft_sync(self, enabled: bool = True):
        ...
        logger.info("Soft sync %s", "enabled" if enabled else "disabled")
    ...
    def set_cross_modulation(self, amount: float):
        ...
        logger.info("Cross-modulation set to %.2f", self.cross_modulation_amount)
    ...
    def enable_sub_oscillator(
        ...
        logger.info(
            "Sub-oscillator %s (octave: %d, waveform: %s)",
            "enabled" if enabled else "disabled",
            self.sub_oscillator_octave,
            self.sub_oscillator_waveform,
        )
    ...
    def set_waveform_morphing(
        ...
        logger.info("Waveform morphing %s", "enabled" if enabled else "disabled")
    ...
    def set_noise_color(self, color: str = "white", level: float = 0.0):
        ...
        logger.info("Noise: %s color, level %.2f", self.noise_color, self.noise_level)
    ...
    def set_filter_type(self, filter_num: int, filter_type: str):
        ...
        logger.info("Filter %d type set to %s", filter_num, filter_type.upper())
    ...
    def set_filter_slope(self, filter_num: int, slope: int):
        ...
        logger.info("Filter %d slope set to %ddB/octave", filter_num, slope)
    ...
    def set_filter_drive(self, filter_num: int, drive: float):
        ...
        logger.info("Filter %d drive set to %.1fx", filter_num, drive)
    ...
    def configure_filter_envelope(
        ...
        logger.info(
            "Filter envelope: amount %.2f, polarity %s",
            self.filter_envelope_amount,
            "positive" if self.filter_envelope_polarity > 0 else "negative",
        )
    ...
    def set_key_tracking(self, amount: float = 0.0, center_note: int = 60):
        ...
        logger.info(
            "Filter key tracking: %.2f (center: %d)",
            self.filter_key_tracking,
            self.filter_key_center,
        )
    ...
    def set_velocity_sensitivity(
        ...
        logger.info(
            "Velocity sensitivity: cutoff %.2f, resonance %.2f (%s)",
            self.velocity_to_cutoff,
            self.velocity_to_resonance,
            self.velocity_curve,
        )
    ...
    def set_input_drive(self, drive: float = 1.0, saturation_type: str = "soft"):
        ...
        logger.info("Input drive: %.1fx (%s saturation)", self.input_drive, self.saturation_type)
    ...
    def set_parallel_routing(self, mix: float = 0.0, separation: float = 0.0):
        ...
        logger.info(
            "Parallel routing: mix %.2f, separation %.2f",
            self.parallel_filter_mix,
            self.filter_separation,
        )
