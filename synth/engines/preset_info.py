...
class PresetInfo:
    ...
    def get_matching_descriptors(self, note: int, velocity: int) -> list[RegionDescriptor]:
        ...
        """
        return [d for d in self.region_descriptors if d.should_play_for_note(note, velocity)]
    ...
    def get_layered_regions(self, note: int, velocity: int) -> list[RegionDescriptor]:
        """
        ...
        for rr_id, group in rr_groups.items():
            if len(group) == 1:
                ...
                best = max(group, key=lambda d: d.get_priority_score(note, velocity))
    ...
    def has_velocity_splits(self) -> bool:
        ...
        for i, d1 in enumerate(self.region_descriptors):
            for d2 in self.region_descriptors[i + 1 :]:
                ...
                keys_overlap = (
                    d1.key_range[0] <= d2.key_range[1] and d2.key_range[0] <= d1.key_range[1]
                )
                # Different velocity ranges?
                vels_different = d1.velocity_range != d2.velocity_range
    ...
    def get_descriptor_by_id(self, region_id: int) -> RegionDescriptor | None:
        """
