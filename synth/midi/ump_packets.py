...
class MIDI2ChannelVoicePacket(UMPPacket):
    ...
    def __init__(self, group: UMPGroup, channel: int, message_type: int, 
                 data_word_1: int, data_word_2: int):
...
class SysExUMP(UMPPacket):
    ...
    def to_words(self) -> List[int]:
        ...
        # Create header word
        header = (self.ump_type << 28) | (self.group << 24) | ((num_words - 1) << 16) | (0x00 if self.complete else 0x01)
        words = [header]
...
class MIDI1ToMIDI2Converter:
    ...
    @staticmethod
    def midi1_to_midi2_channel_voice(status_byte: int, data1: int, data2: int) -> MIDI2ChannelVoicePacket:
