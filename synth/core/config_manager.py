                        ...
                        if isinstance(key, str) and ':' in key:
                            try:
                                ...
                                if isinstance(value, str) and ':' in value:
                                    ...
                                    parsed['remap'][(from_bank, from_prog)] = (int(to_bank), int(to_prog))
                                elif isinstance(value, (list, tuple)) and len(value) >= 2:
                                    parsed['remap'][(from_bank, from_prog)] = (int(value[0]), int(value[1]))
                            except (ValueError, IndexError):
                                pass
                    ...
                    if isinstance(key, str) and ':' in key:
                        try:
                            ...
                            if isinstance(value, (list, tuple)) and len(value) >= 2:
                                result['remap'][(int(from_bank), int(from_prog))] = (int(value[0]), int(value[1]))
                            elif isinstance(value, str) and ':' in value:
                                to_bank, to_prog = value.split(':')
                                result['remap'][(int(from_bank), int(from_prog))] = (int(to_bank), int(to_prog))
                        except (ValueError, IndexError):
                            pass
