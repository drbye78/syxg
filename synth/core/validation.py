...
class AudioValidator:
    ...
    def validate_buffer(self, buffer: np.ndarray,
                       expected_channels: int = None,
                       expected_dtype: np.dtype = None) -> ValidationResult:
        """
    ...
    def validate_parameter_range(self, param_value: float,
    ...
    def validate_midi_message(self, message_bytes: bytes) -> ValidationResult:
        ...
        if expected_length > 0 and len(message_bytes) != expected_length:
            result.add_error(ValidationError(
                ...
                "INVALID_MIDI_LENGTH",
                {"expected": expected_length, "actual": len(message_bytes), "status": status_byte}
            ))
    ...
    def _get_expected_midi_length(self, status_byte: int) -> int:
        ...
        # Channel messages
        if status_byte & 0xF0 in (0x80, 0x90, 0xA0, 0xB0, 0xE0):  # Note off/on, poly pressure, control, pitch bend
            return 3
    ...
    def validate_audio_system(self, sample_rate: int, block_size: int,
                            max_channels: int) -> ValidationResult:
        """
    ...
    def validate_parameter(self, name: str, value: Any) -> ValidationResult:
        ...
        if not isinstance(value, param_def['type']):
            result.add_error(ValidationError(
                ...
                "INVALID_PARAMETER_TYPE",
                {"parameter": name, "expected": param_def['type'].__name__, "actual": type(value).__name__}
            ))
            return result

        # Range checking
        range_result = self.validate_parameter_range(value, param_def['min'], param_def['max'], name)
