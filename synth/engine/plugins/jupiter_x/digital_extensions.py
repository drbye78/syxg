...
class JupiterXDigitalPlugin(SynthesisFeaturePlugin):
    ...
    def _linear_morph(
        self, wavetable1: np.ndarray, wavetable2: np.ndarray, morph_factor: float
    ) -> np.ndarray:
    ...
    def apply_morphing(
        self, source_wavetable: str, target_wavetable: str, morph_factor: float
    ) -> np.ndarray | None:
        """
    ...
    def enable_wavetable_scanning(
        self, enabled: bool = True, speed: float = 1.0, modulation_depth: float = 0.0
    ):
        ...
        if self.wavetable_engine and hasattr(self.wavetable_engine, "enable_scanning"):
            self.wavetable_engine.enable_scanning(
    ...
    def enable_spectral_processing(
        self,
        enabled: bool = True,
        fft_size: int = 2048,
        enhancement: float = 0.0,
        harmonic_focus: float = 0.5,
    ):
        ...
        if self.wavetable_engine and hasattr(self.wavetable_engine, "enable_spectral_processing"):
            self.wavetable_engine.enable_spectral_processing(
    ...
    def enable_wavetable_ring_modulation(
        ...
        if self.wavetable_engine and hasattr(self.wavetable_engine, "enable_ring_modulation"):
            self.wavetable_engine.enable_ring_modulation(
    ...
    def enable_harmonic_processing(
        self,
        enabled: bool = True,
        boost_factors: list[float] | None = None,
        ...
        if suppression_factors:
            self.harmonic_suppression = list(suppression_factors[:16]) + [1.0] * (
                16 - len(suppression_factors)
            )

        if self.wavetable_engine and hasattr(self.wavetable_engine, "enable_harmonic_processing"):
            self.wavetable_engine.enable_harmonic_processing(
    ...
    def apply_spectral_enhancement(self, wavetable_data: np.ndarray) -> np.ndarray:
        ...
        if self.harmonic_focus != 0.5:
            ...
            focus_curve = 1.0 + (self.harmonic_focus - 0.5) * 2.0 * np.sin(
                np.pi * harmonic_positions
            )
    ...
    def generate_multi_layer_wavetable(
        self, base_wavetable: np.ndarray, layer_count: int = 4
    ) -> np.ndarray:
        ...
        for i in range(1, min(layer_count, len(self.layer_allocation))):
            if self.layer_allocation[i]:
                ...
                layer_data = base_wavetable * np.sin(
                    np.arange(len(base_wavetable)) * 0.01 + phase_shift
                )
    ...
    def process_wavetable_scanning(
        self, base_output: np.ndarray, lfo_value: float = 0.0
    ) -> np.ndarray:
    ...
    def create_custom_wavetable(
        self, name: str, size: int = 2048, waveform_type: str = "sine"
    ) -> bool:
