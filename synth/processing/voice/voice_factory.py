...
from ...engines.synthesis_engine import SynthesisEngine, SynthesisEngineRegistry
...
class VoiceFactory:
    ...
    __slots__ = ["_preset_cache", "engine_registry", "synth"]

    def __init__(self, engine_registry: SynthesisEngineRegistry, synth: Any | None = None):
        """
    ...
    def create_voice(self, bank: int, program: int, channel: int, sample_rate: int) -> Voice | None:
        ...
        for engine_type in self.engine_registry.get_priority_order():
            ...
            try:
                ...
                preset_info = self._get_preset_info_cached(engine, bank, program)
        ...
        logger.warning(f"No engine can handle program {program} in bank {bank}")
        return None

    def _get_preset_info_cached(
        self, engine: SynthesisEngine, bank: int, program: int
    ) -> PresetInfo | None:
        """
        ...
        for engine_type in self.engine_registry.get_priority_order():
            ...
            if hasattr(engine, "get_available_programs"):
                try:
                    ...
                    logger.warning(f"Engine {engine_type} failed to get programs: {e}")

        return programs

    def get_program_info(self, bank: int, program: int) -> dict[str, Any] | None:
        """
    ...
    def preload_preset(self, bank: int, program: int) -> bool:
        """
...
def create_voice_factory(
    engine_registry: SynthesisEngineRegistry, synth: Any | None = None
) -> VoiceFactory:
    """
