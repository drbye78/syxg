...
class MIDI2EffectProcessor:
    ...
    def create_effect(
        ...
        if effect_type in [
            MIDI2EffectType.REVERB_HALL_32BIT,
            MIDI2EffectType.REVERB_ROOM_32BIT,
            MIDI2EffectType.REVERB_PLATE_32BIT,
            MIDI2EffectType.REVERB_CHAMBER_32BIT,
            MIDI2EffectType.REVERB_SPRING_32BIT,
            MIDI2EffectType.REVERB_CONVOLUTION_32BIT,
        ]:
            from .reverb import ReverbEffect

            effect_instance = ReverbEffect(self.sample_rate, effect_type=effect_type.name)
        elif effect_type in [
            MIDI2EffectType.CHORUS_STANDARD_32BIT,
            MIDI2EffectType.CHORUS_FLANGER_32BIT,
            MIDI2EffectType.CHORUS_CELESTE_32BIT,
            MIDI2EffectType.CHORUS_DIMENSION_32BIT,
            ...
            MIDI2EffectType.DELAY_STEREO_32BIT,
            MIDI2EffectType.DELAY_MULTI_TAP_32BIT,
            MIDI2EffectType.DELAY_CROSS_32BIT,
            MIDI2EffectType.DELAY_MODULATED_32BIT,
            MIDI2EffectType.DELAY_ANALOG_32BIT,
            MIDI2EffectType.DELAY_DIGITAL_32BIT,
        ]:
            from .delay import DelayEffect

            effect_instance = DelayEffect(self.sample_rate, effect_type=effect_type.name)
        elif effect_type in [
            MIDI2EffectType.FILTER_LOW_PASS_32BIT,
            MIDI2EffectType.FILTER_HIGH_PASS_32BIT,
            MIDI2EffectType.FILTER_BAND_PASS_32BIT,
            MIDI2EffectType.FILTER_NOTCH_32BIT,
            MIDI2EffectType.FILTER_ALL_PASS_32BIT,
            MIDI2EffectType.FILTER_FORMANT_32BIT,
            MIDI2EffectType.FILTER_STATE_VARIABLE_32BIT,
            MIDI2EffectType.FILTER_RESONANT_32BIT,
        ]:
            from .filter import FilterEffect

            effect_instance = FilterEffect(self.sample_rate, effect_type=effect_type.name)
        elif effect_type in [
            MIDI2EffectType.MODULATION_TREMOLO_32BIT,
            MIDI2EffectType.MODULATION_VIBRATO_32BIT,
            MIDI2EffectType.MODULATION_AUTO_PANNER_32BIT,
            MIDI2EffectType.MODULATION_PHASER_32BIT,
            MIDI2EffectType.MODULATION_FLANGER_32BIT,
            MIDI2EffectType.MODULATION_ROTARY_SPEAKER_32BIT,
        ]:
            from .modulation import ModulationEffect

            effect_instance = ModulationEffect(self.sample_rate, effect_type=effect_type.name)
        elif effect_type in [
            MIDI2EffectType.DYNAMICS_COMPRESSOR_32BIT,
            MIDI2EffectType.DYNAMICS_LIMITER_32BIT,
            MIDI2EffectType.DYNAMICS_EXPANDER_32BIT,
            MIDI2EffectType.DYNAMICS_GATE_32BIT,
        ]:
            from .dynamics import DynamicsEffect

            effect_instance = DynamicsEffect(self.sample_rate, effect_type=effect_type.name)
        elif effect_type in [
            MIDI2EffectType.DISTORTION_SOFT_CLIP_32BIT,
            MIDI2EffectType.DISTORTION_HARD_CLIP_32BIT,
            MIDI2EffectType.DISTORTION_FUZZ_32BIT,
            MIDI2EffectType.DISTORTION_OVERDRIVE_32BIT,
            ...
            MIDI2EffectType.EQ_PARAMETRIC_32BIT,
            MIDI2EffectType.EQ_GRAPHIC_32BIT,
            MIDI2EffectType.EQ_SHELVING_32BIT,
            MIDI2EffectType.EQ_HIGH_PASS_32BIT,
        ]:
            from .eq import EQEffect

            effect_instance = EQEffect(self.sample_rate, effect_type=effect_type.name)
        elif effect_type in [
            MIDI2EffectType.PITCH_SHIFTER_32BIT,
            MIDI2EffectType.HARMONIZER_32BIT,
            MIDI2EffectType.VOCODER_32BIT,
            MIDI2EffectType.RING_MODULATOR_32BIT,
        ]:
    ...
    def _create_specialized_effect(self, effect_type: MIDI2EffectType):
        ...
        if effect_type in [
            ...
            MIDI2EffectType.PER_NOTE_DELAY_SEND_32BIT,
            MIDI2EffectType.PER_NOTE_DISTORTION_SEND_32BIT,
        ]:
            ...
            MIDI2EffectType.PER_NOTE_FILTER_CUTOFF_32BIT,
            MIDI2EffectType.PER_NOTE_FILTER_RESONANCE_32BIT,
            MIDI2EffectType.PER_NOTE_PAN_32BIT,
            MIDI2EffectType.PER_NOTE_WIDTH_32BIT,
        ]:
            ...
            MIDI2EffectType.SPECTRAL_32BIT,
            MIDI2EffectType.CONVOLUTION_REVERB_32BIT,
        ]:
    ...
    def get_effect_parameter(
        self, effect_id: int, param_name: str, note: int | None = None
    ) -> float:
        """
        ...
        if note is not None:
            # Per-note parameter
            return (
                self.per_note_effect_parameters.get(note, {})
                .get(effect_id, {})
                .get(param_name, 0.0)
            )
        else:
...
class XGMIDI2EffectsProcessor(MIDI2EffectProcessor):
    ...
    def _initialize_xg_effects(self):
        ...
        if self.system_reverb_id is not None:
            ...
            self.set_effect_parameter(
                self.system_reverb_id, "early_reflection", 0.7, resolution_bits=32
            )
        ...
        if self.system_variation_id is not None:
            ...
            self.set_effect_parameter(
                self.system_variation_id, "high_freq_damp", 0.3, resolution_bits=32
            )
    ...
    def _set_xg_reverb_parameter(self, address: int, value: float):
        ...
        if address == 0x000000:  # REV TYPE
            ...
            self.set_effect_parameter(
                self.system_reverb_id, "time", value * 10.0, resolution_bits=32
            )  # Scale to 0-10s
        elif address == 0x000002:  # REV PRE-DELAY TIME
            self.set_effect_parameter(
                self.system_reverb_id, "pre_delay", value * 0.5, resolution_bits=32
            )  # Scale to 0-0.5s
        elif address == 0x000003:  # REV PRE-DELAY FEED
            self.set_effect_parameter(
                self.system_reverb_id, "pre_delay_feedback", value, resolution_bits=32
            )
        elif address == 0x000004:  # REV ER LEVEL
            self.set_effect_parameter(
                self.system_reverb_id, "early_reflection_level", value, resolution_bits=32
            )
    ...
    def _set_xg_chorus_parameter(self, address: int, value: float):
        ...
        if address == 0x000100:  # CHO TYPE
            ...
            self.set_effect_parameter(
                self.system_chorus_id, "pre_delay", value * 0.1, resolution_bits=32
            )  # Scale to 0-0.1s
        elif address == 0x000105:  # CHO RATE
            self.set_effect_parameter(
                self.system_chorus_id, "rate", value * 10.0, resolution_bits=32
            )  # Scale to 0-10 Hz
