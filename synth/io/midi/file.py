...
class FileParser:
    ...
    def _convert_utility_packet_to_message(self, packet) -> MIDIMessage | None:
        ...
        if packet.utility_type == 0x1:  # JR Timestamp
            return MIDIMessage(
                type="jitter_reduction_timestamp", data={"timestamp": packet.data}, timestamp=0.0
            )
    ...
    def _parse_track(self, track_data: bytes, track_index: int) -> list[MIDIMessage]:
        ...
        while offset < len(track_data):
            ...
            if event_type == MIDIStatus.META_EVENT:
                ...
                event_type == MIDIStatus.SYSTEM_EXCLUSIVE
                or event_type == MIDIStatus.END_OF_EXCLUSIVE
            ):
                # System Exclusive
                length, offset = self._read_variable_length(track_data, offset)
                if offset + length > len(track_data):
                ...
                message = MIDIMessage(
                    type="sysex", timestamp=time_seconds, data={"raw_data": sysex_data}
                )
                messages.append(message)
                ...
                message = self._parse_channel_event(
                    running_status, track_data, offset, time_seconds
                )
                if message:
                    messages.append(message)
    ...
    def _parse_meta_event(
        self, meta_type: int, data: list[int], timestamp: float
    ) -> MIDIMessage | None:
        """Parse MIDI meta event."""
        if meta_type == 0x00 and len(data) == 2:  # Sequence Number
            seq_num = (data[0] << 8) | data[1]
            return MIDIMessage(
                type="sequence_number", timestamp=timestamp, data={"sequence_number": seq_num}
            )
        elif meta_type == 0x01:  # Text Event
            text = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(type="text", timestamp=timestamp, data={"text": text})
        elif meta_type == 0x02:  # Copyright Notice
            copyright_text = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(type="copyright", timestamp=timestamp, data={"text": copyright_text})
        elif meta_type == 0x03:  # Sequence/Track Name
            track_name = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(type="track_name", timestamp=timestamp, data={"name": track_name})
        elif meta_type == 0x04:  # Instrument Name
            instrument_name = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(
                type="instrument_name", timestamp=timestamp, data={"name": instrument_name}
            )
        elif meta_type == 0x05:  # Lyric
            lyric_text = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(type="lyric", timestamp=timestamp, data={"text": lyric_text})
        elif meta_type == 0x06:  # Marker
            marker_text = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(type="marker", timestamp=timestamp, data={"text": marker_text})
        elif meta_type == 0x07:  # Cue Point
            cue_text = bytes(data).decode("utf-8", errors="ignore")
            return MIDIMessage(type="cue_point", timestamp=timestamp, data={"text": cue_text})
        elif meta_type == 0x20 and len(data) == 1:  # MIDI Channel Prefix
            channel = data[0]
            return MIDIMessage(
                type="channel_prefix", timestamp=timestamp, data={"channel": channel}
            )
            ...
            return MIDIMessage(
                type="tempo", timestamp=timestamp, data={"tempo_us_per_beat": tempo_us}
            )
            ...
            return MIDIMessage(
                type="smpte_offset", timestamp=timestamp, data={"smpte_seconds": smpte_seconds}
            )
            ...
            return MIDIMessage(
                type="key_signature",
                timestamp=timestamp,
                data={"key": key, "scale": "major" if scale == 0 else "minor"},
            )
        elif meta_type == 0x7F:  # Sequencer Specific Meta Event
            return MIDIMessage(
                type="sequencer_specific", timestamp=timestamp, data={"raw_data": data}
            )

        # For any other meta events, return a generic meta message
        return MIDIMessage(
            type="meta", timestamp=timestamp, data={"meta_type": meta_type, "data": data}
        )

    def _parse_channel_event(
        self, status: int, data: bytes, offset: int, timestamp: float
    ) -> MIDIMessage | None:
        ...
        return MIDIMessage(
            type=message_type, channel=channel, timestamp=timestamp, data=message_data
        )
