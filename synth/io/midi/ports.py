...
Example:
    >>> from synth.io.midi import get_input_names, open_input
...
# Try to import rtmidi (python-rtmidi is the Python 3.14 compatible fork)
try:
    import rtmidi
    ...
    rtmidi = None
    logger.warning(
        "rtmidi not installed - MIDI port I/O disabled. Install with: pip install python-rtmidi"
    )
        ...
        try:
            if self.virtual:
                ...
                try:
                    ...
                    raise ValueError(
                        f"MIDI output port not found: {self.name}. Available ports: {port_names}"
                    )
...
def open_input(
    name: str | None = None, callback: Callable[[MIDIMessage], None] | None = None
) -> MIDIInputPort:
    """
    Open a MIDI input port.
