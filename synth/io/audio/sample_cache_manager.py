...
class SampleCacheManager:
    ...
    def get_sample(
        self, source_id: str, sample_id: int, loader: Callable[[], np.ndarray | None]
    ) -> np.ndarray | None:
        """
    ...
    def _cache_sample(
        self, key: tuple[str, int], sample_data: np.ndarray, source_id: str, sample_id: int
    ) -> None:
        """
        ...
        with self._lock:
            ...
            while self._current_memory_bytes + sample_bytes > self.max_memory_bytes:
            ...
            self._cache[key] = CachedSample(
                data=sample_data, size_bytes=sample_bytes, source_id=source_id, sample_id=sample_id
            )
            self._access_order.append(key)
    ...
    def get_cached_samples_for_source(self, source_id: str) -> list[int]:
        ...
        """
        with self._lock:
            return [sample_id for (src, sample_id) in self._cache.keys() if src == source_id]

    def preload_samples(
        self, samples: list[tuple[str, int, Callable[[], np.ndarray | None]]]
    ) -> int:
        """
