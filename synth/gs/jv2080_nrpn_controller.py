...
class JV2080NRPNController:
    ...
    def _build_parameter_map(self) -> Dict[Tuple[int, int], Dict[str, Any]]:
        ...
        system_params = {
            ...
            0x03: {'name': 'master_coarse_tune', 'range': (-24, 24), 'default': 0, 'unit': 'semitones'},
        ...
        part_param_names = [
            'instrument_lsb', 'instrument_msb', 'volume', 'pan',
            'coarse_tune', 'fine_tune', 'reverb_send', 'chorus_send',
            'delay_send', 'mfx_send', 'insert_effect_assign', 'key_range_low',
            'key_range_high', 'velocity_range_low', 'velocity_range_high',
            'receive_channel', 'polyphony_mode', 'portamento_time',
            'reserved_18', 'reserved_19', 'reserved_1A', 'reserved_1B',
            'reserved_1C', 'reserved_1D', 'reserved_1E', 'reserved_1F',
            'reserved_20', 'reserved_21', 'reserved_22', 'reserved_23',
            'reserved_24', 'reserved_25'
        ]
        ...
        reverb_params = {
            ...
            0x05: {'name': 'reverb_high_freq_damp', 'range': (0, 127), 'default': 64, 'unit': 'freq'},
    ...
    def _process_nrpn_data(self, data_value: int) -> bool:
        ...
        elif param_name.startswith('system_') or param_name in ['master_tune', 'master_volume', 'master_pan',
                                                              'master_coarse_tune', 'master_fine_tune',
                                                              'reverb_send_level', 'chorus_send_level',
                                                              'delay_send_level', 'mfx_send_level',
                                                              'device_id', 'midi_channel', 'local_control',
                                                              'program_change_mode', 'lcd_contrast', 'led_brightness']:
            # System parameter
    ...
    def _set_effects_parameter(self, param_name: str, value: int) -> bool:
        ...
        if param_name.startswith('reverb_'):
            ...
            if addr_low is not None:
                return self.component_manager.process_parameter_change(bytes([0x03, addr_low]), value)
            ...
            addr_low = param_map.get(param_name)
            if addr_low is not None:
                return self.component_manager.process_parameter_change(bytes([0x04, addr_low]), value)
            ...
            if param_name == 'mfx_type':
                ...
                return self.component_manager.process_parameter_change(bytes([0x05, param_num]), value)
