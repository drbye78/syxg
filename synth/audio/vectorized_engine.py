"""
VECTORIZED AUDIO ENGINE - PHASE 2 PERFORMANCE

Handles audio block generation and processing with vectorized NumPy operations.
...
3. BATCH PROCESSING - Processes multiple samples simultaneously rather than individually
...
class VectorizedAudioEngine:
    ...
    def _ensure_buffer_size(self, block_size: int):
        ...
        if block_size > len(self.left_buffer):
            ...
            self.channel_buffers = np.resize(self.channel_buffers, (self.num_channels, new_size, 2))
    ...
    def generate_audio_block_vectorized(self, channel_renderers: List, effect_manager,
        ...
        with self.lock:
            ...
            if active_renderers:
                ...
                try:
                    # VECTORIZED BATCH CHANNEL PROCESSING
                    left_block, right_block = self._generate_channel_audio_vectorized(
                        active_renderers, block_size
                    )
                    
                    # Copy vectorized data to output buffers using vectorized operations
                    ...
                    # Fallback to per-channel processing if vectorized processing fails
                    # print(f"Vectorized processing failed, falling back to per-channel processing: {e}")
            ...
            if effect_manager:
                return self._apply_effects_vectorized(effect_manager, block_size)
            else:
    ...
    def _generate_channel_audio_vectorized(self, active_renderers: List, 
        ...
        """
        VECTORIZED CHANNEL AUDIO GENERATION - PHASE 2 PERFORMANCE
        
        Generate audio for all active channels simultaneously using vectorized NumPy operations.
        
        Performance optimizations:
        1. BATCH PROCESSING - Processes all channels with vectorized operations
        ...
        for renderer in active_renderers:
            try:
                ...
                if hasattr(renderer, 'generate_sample_block_vectorized'):
                    ...
                    np.add(left_batch, self.temp_left[:block_size], out=left_batch)
                    np.add(right_batch, self.temp_right[:block_size], out=right_batch)
    ...
    def _generate_channel_audio_per_channel(self, active_renderers: List, block_size: int):
        """
        PER-CHANNEL AUDIO GENERATION WITH VECTORIZED OPERATIONS - FALLBACK METHOD
        
        Generate audio for each channel individually with vectorized operations.
        ...
        Args:
            ...
            block_size: Block size in samples
        """
        # Process each active renderer with vectorized operations
        for renderer in active_renderers:
            try:
                # Clear temporary buffers using vectorized operations
                self.temp_left[:block_size].fill(0.0)
    ...
    def _apply_effects_vectorized(self, effect_manager, block_size: int) -> Tuple[np.ndarray, np.ndarray]:
        """
        VECTORIZED EFFECTS PROCESSING - PHASE 2 PERFORMANCE
        
        Apply effects to final mixed stereo output using vectorized operations.
        ...
        2. NUMPY-BASED OPERATIONS - Uses NumPy for efficient effect processing
    ...
    def _mix_channels_without_effects(self, block_size: int) -> Tuple[np.ndarray, np.ndarray]:
        """
        MIX CHANNELS WITHOUT EFFECTS PROCESSING - VECTORIZED MIXING
        
        Mix all channels into single stereo output without effects processing.
        
        Performance optimizations:
        1. NUMPY-BASED OPERATIONS - Uses NumPy for efficient audio mixing
        2. PRE-ALLOCATED BUFFERS - Uses pre-allocated buffers to reduce allocation overhead
        3. ZERO-CLEARING OPTIMIZATION - Clears buffers efficiently using vectorized operations
    ...
    def reset(self):
        ...
        with self.lock:
            ...
            self.channel_buffers.fill(0.0)
            self.effect_input.fill(0.0)
            self.effect_output.fill(0.0)
