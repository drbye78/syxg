...
class VectorizedModulationRoute:
    ...
    def get_modulation_value_vectorized(
        self, source_values: np.ndarray, velocities: np.ndarray, notes: np.ndarray
    ) -> np.ndarray:
        """
...
class VectorizedModulationMatrix:
    ...
    def set_route(
        ...
        self.routes[index] = VectorizedModulationRoute(
            source, destination, amount, polarity, velocity_sensitivity, key_scaling
        )

        # Initialize destination array for this route if not already present
        if destination not in self._dest_arrays:
            self._dest_arrays[destination] = np.zeros(
                self._max_block_size or 1024, dtype=np.float32
            )
    ...
    def process_vectorized(
        self, sources: dict[str, np.ndarray], velocities: np.ndarray, notes: np.ndarray
    ) -> dict[str, np.ndarray]:
        """
        ...
        for route in self.routes:
            ...
            if len(source_values) != block_size:
                if len(source_values) == 1:
                    ...
                    raise ValueError(
                        f"Source array size {len(source_values)} doesn't match block size {block_size}"
                    )
