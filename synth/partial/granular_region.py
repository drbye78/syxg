...
class GranularRegion(IRegion):
    ...
    __slots__ = [
        '_max_clouds', '_grain_density', '_grain_duration',
        '_time_stretch', '_pitch_shift', '_position_spread',
        '_playback_mode', '_source_buffer', '_grain_clouds',
        '_cloud_positions', '_source_length'
    ]
    ...
    def _load_sample_data(self) -> Optional[np.ndarray]:
        ...
        if sample_id is None:
            ...
            self._source_buffer = np.random.uniform(-0.1, 0.1, int(self.sample_rate * 2)).astype(np.float32)
    ...
    def _create_grain_clouds(self) -> None:
        ...
        try:
            from ..engine.granular_engine import GrainCloud
