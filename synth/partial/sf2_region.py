...
class SF2Region(IRegion):
    ...
    __slots__ = [
        ...
        '_key_range', '_velocity_range', '_exclusive_class'
    ]
    ...
    def _load_sample_data(self) -> Optional[np.ndarray]:
        ...
        try:
            ...
            logger.warning(
                f"Failed to load SF2 sample {self.descriptor.sample_id}"
            )
            return None
    ...
    def _get_sf2_zone(self) -> Optional[Any]:
        ...
        if self.soundfont_manager and hasattr(self.soundfont_manager, 'get_zone'):
            try:
                self._sf2_zone = self.soundfont_manager.get_zone(
                    self.descriptor.region_id
                )
    ...
    def _generate_samples_with_mipmap(
        self, 
        output: np.ndarray, 
        block_size: int
    ) -> None:
        ...
        for i in range(block_size):
            ...
            output[i * 2 + 1] = sample  # Right (mono for now)
