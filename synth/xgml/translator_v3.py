...
class SF2EngineTranslator(EngineTranslator):
    ...
    def translate(self, config: dict[str, Any]) -> list[MIDIMessage]:
        ...
        if "program" in config:
            ...
            messages.append(
                MIDIMessage(type="program_change", channel=0, data={"program": program})
            )
        ...
        if "bank" in config:
            ...
            # Bank MSB
            messages.append(
                MIDIMessage(type="control_change", channel=0, data={"controller": 0, "value": bank})
            )
...
class FMXEngineTranslator(EngineTranslator):
    ...
    def translate(self, config: dict[str, Any]) -> list[MIDIMessage]:
        ...
        if "algorithm" in config:
            ...
            # Map algorithm names to XG/GM2 operator counts
            algorithm_ops = {
                "fm_op4": 4,
                "fm_op6": 6,
                "fm_op8": 8,
            ...
            ops = algorithm_ops.get(algorithm.lower(), 4)

            # FM algorithm via CC 80-83 (varies by manufacturer)
            ...
            messages.append(
                MIDIMessage(
                    type="control_change", channel=0, data={"controller": 80, "value": algo_value}
                )
            )
        ...
        if "master_volume" in config:
            ...
            messages.append(
                MIDIMessage(
                    type="control_change", channel=0, data={"controller": 7, "value": volume}
                )
            )
    ...
    def _generate_rpn_sequence(
        self, channel: int, msb: int, lsb: int, value: int
    ) -> list[MIDIMessage]:
        """Generate RPN parameter sequence."""
        return [
            MIDIMessage(
                type="control_change", channel=channel, data={"controller": 101, "value": msb}
            ),  # RPN MSB
            MIDIMessage(
                type="control_change", channel=channel, data={"controller": 100, "value": lsb}
            ),  # RPN LSB
            MIDIMessage(
                type="control_change", channel=channel, data={"controller": 6, "value": value}
            ),  # Data
        ]
...
class PhysicalEngineTranslator(EngineTranslator):
    ...
    def translate(self, config: dict[str, Any]) -> list[MIDIMessage]:
        ...
        if model_type == "string":
            ...
            messages.extend(
                self._translate_percussion_model(config.get("percussion_parameters", {}))
            )
...
class SpectralEngineTranslator(EngineTranslator):
    ...
    def translate(self, config: dict[str, Any]) -> list[MIDIMessage]:
        ...
        if "gain" in filtering:
            ...
            messages.append(
                MIDIMessage(
                    type="control_change", channel=0, data={"controller": 74, "value": gain_val}
                )
            )
        ...
        if "output_gain" in output_proc:
            ...
            messages.append(
                MIDIMessage(
                    type="control_change", channel=0, data={"controller": 7, "value": gain_val}
                )
            )
...
class WorkstationTranslator:
    ...
    def _generate_nrpn_sequence(
        ...
        return [
            MIDIMessage(
                type="control_change", channel=actual_channel, data={"controller": 99, "value": msb}
            ),  # NRPN MSB
            MIDIMessage(
                type="control_change", channel=actual_channel, data={"controller": 98, "value": lsb}
            ),  # NRPN LSB
            MIDIMessage(
                type="control_change",
                channel=actual_channel,
                data={"controller": 6, "value": value},
            ),  # Data
        ]
...
class EffectsTranslator:
    ...
    def _translate_system_effects(self, effects: dict[str, Any]) -> list[MIDIMessage]:
        ...
        if "chorus" in effects:
            ...
            if "algorithm" in chorus:
                algorithm_map = {"chorus_1": 0, "chorus_2": 1, "celeste": 2, "flanger": 3}
    ...
    def _translate_variation_effects(self, effects: list[dict[str, Any]]) -> list[MIDIMessage]:
        ...
        for effect in effects:
            ...
            if "parameters" in effect:
                ...
                if "depth" in params:
                    messages.extend(
                        self._generate_nrpn_sequence(None, 3, 1, int(params["depth"] * 127))
                    )
                ...
                for band_name, (gain_msb, gain_lsb, freq_msb, freq_lsb) in band_nrpn_map.items():
                    if band_name in bands:
                        ...
                        if "gain" in band:
                            ...
                            messages.extend(
                                self._generate_nrpn_sequence(None, gain_msb, gain_lsb, gain_val)
                            )
    ...
    def _generate_nrpn_sequence(
        ...
        return [
            MIDIMessage(
                type="control_change", channel=actual_channel, control=99, value=msb, time=0.0
            ),  # NRPN MSB
            MIDIMessage(
                type="control_change", channel=actual_channel, control=98, value=lsb, time=0.0
            ),  # NRPN LSB
            MIDIMessage(
                type="control_change", channel=actual_channel, control=6, value=value, time=0.0
            ),  # Data
        ]
    ...
    def _translate_performance_controls(self, controls_config: dict[str, Any]) -> list[MIDIMessage]:
        ...
        for slider_name, slider_config in sliders.items():
            if "default" in slider_config and "midi_cc" in slider_config:
                ...
                cc_value = (
                    int(default_value * 127) if isinstance(default_value, float) else default_value
                )
                messages.append(
                    MIDIMessage(
                        type="control_change",
